...
class ActionApplicatorV2:
    ...
    def _apply_reassign_item(self, state: ProjectState, rec: Recommendation) -> None:
        ...
        for item in state.work_items:
            ...
            # Reassignment changes ownership only. Resource speed/capacity is evaluated
            # by the canonical ResourceIntelligence model during recomputation.
            item.assigned_resource = resource_id
    ...
    def _apply_split_item(self, state: ProjectState, rec: Recommendation) -> None:
        ...
        for item in list(state.work_items):
            ...
            original_remaining = float(item.remaining_effort_hrs)
            half_hours = original_hours / 2.0
            half_remaining = original_remaining / 2.0

            # Derive both siblings from immutable pre-mutation snapshots.
            item.current_estimate_hrs = half_hours
            item.remaining_effort_hrs = half_remaining
            ...
            new_item.remaining_effort_hrs = half_remaining
    ...
    def _apply_rebalance_sprint_load(self, state: ProjectState, rec: Recommendation) -> None:
        ...
        # Do not mutate contractual allocation percentages to manufacture capacity.
        # Recomputed sprint loads reflect the moved work on both source and receiver.
